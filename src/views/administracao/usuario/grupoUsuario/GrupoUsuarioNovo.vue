  ...
  <div class="m-3">
    ...
    <div class="card card-body mx-2">
      <form ref="form" @submit.prevent="submitForm" >
        <div class="row">
          <s-input-zoom
            v-model="object.idGroup"
            ref="idGroup"
            divClass="col-12 col-md-12"
            label="Grupo"
            required>
            <template #default>
              <Grupo :zoom="true" @selectedItem="handleSelectedGroup"/>
            </template>
          </s-input-zoom>
        </div>
        <div class="row">
...
import Grupo from '@/views/administracao/grupo/Grupo.vue'
...
export default {
  ...
  methods: {
    async loadItem(id) {
      if (await this.$checkSession()) {
        ...
        .catch((err) => {
          console.error(err)
          this.$router.push({ name: 'userGroupChildren'})
        })
      }
    ...
    loadIdUser() {
      this.object.idUser = this.$route.params.idUser
    },

    handleSelectedGroup(item) {
      this.$refs.idGroup.modalZoom.hide()
      this.object.idGroup = item.id.toString()
    },
  ...
  mounted() {
    this.loadIdUser();
    this.$route.name == 'userGroupUpdate' ? this.title = 'Edição de Grupo' : this.title = 'Cadastro de Grupo'
