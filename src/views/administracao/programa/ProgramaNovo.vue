...
export default {
        ...
        if (await validateForm(this.$refs.form)) {
          ...
          if (result.status) {
            if (result.status != '204') {
              ...
              this.$store.dispatch('setToastMessage', 'Programa criado com sucesso !')
              this.object = {}
            }
          }
        ...
        if (this.object.id) {
          ...
          if (result.status) {
            if (result.status != '204') {
              ...
              this.$store.dispatch('setShowToast', true)
              this.$store.dispatch('setToastMessage', 'Programa alterado com sucesso !')
              this.$router.back()
            }
          }
  ...
  mounted() {
    this.$route.name == 'screenUpdate' ? this.title = 'Edição de Programa' : this.title = 'Cadastro de Programa'
